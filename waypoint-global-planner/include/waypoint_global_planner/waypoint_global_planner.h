...
public:
   ...
   * @param path The input path to be used for the creation of visualization
   * markers
   */
  void createAndPublishMarkersFromPath(
      const std::vector<geometry_msgs::PoseStamped> &path);

  /**
   * @brief Interpolates a path (position and orientation) using a fixed number
   * of points per meter
...
private:
  ...
  double epsilon_; //!< distance threshold between two waypoints that signifies
                   //!< the last waypoint
  int waypoints_per_meter_; //!< number of waypoints per meter of generated path
